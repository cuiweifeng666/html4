...
$(document).ready(function(){
	...
	var rotateFn = function(item,txt){
		...
		$('.rotate-area').rotate({
			...
			step:function(){
				n++;
				console.log(getmatrix($('.rotate-area').css('transform')))
			},
			duration:8000,
			callback:function(){
	...
	var val= a.slice(7).split(')').join('').split(',')   
        var aa=Math.round(180*Math.asin(val[0])/ Math.PI);  

        var bb=Math.round(180*Math.acos(val[1])/ Math.PI);  
        var cc=Math.round(180*Math.asin(val[2])/ Math.PI);  
        var dd=Math.round(180*Math.acos(val[3])/ Math.PI);
